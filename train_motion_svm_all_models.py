...
def run_adaptive_grid_search(pose, estimator, X, y, groups, output_name, static_gestures=None):
    ...
    print(f"🔧 Search space: {len(params['kernels'])} kernels × {len(params['Cs'])} C × {len(params['gammas'])} gamma = {len(params['kernels']) * len(params['Cs']) * len(params['gammas'])} combinations")
